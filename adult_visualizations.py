...
def model(cleanedDF):
    ...
    column_train, column_test, class_train, class_test = train_test_split(column_labels, class_labels, test_size=0.2,
                                                                          random_state=1)
    ...
    importance = {'marital status': marital_status_importance, 'age': importance_dict['age'],
                  'education number': importance_dict['education_num'],
                  'hours per week': importance_dict['hours_per_week'], 'occupation': occupation_importance,
                  'workclass': workclass_importance, 'relationship': relationship_importance,
                  'native country': native_country_importance,
    ...
    colors = ["#e5ac77", "#3f4d53", "#fec15c", "#c5c5c6", "#e1e18d", "#7d807e", "#e8afaf", "#fef9f0", "#f77059",
              "#fbef10", "#9c8187"]
...
# creates a vertical, single bar chart showing the percent of individuals who make more than 50k
def create_vert_bar(df, column):
    unique_val = pd.unique(df[column])
    condensedDF = df[['income', column]]
    high_inc = condensedDF.loc[df['income'] == 1]
    low_inc = condensedDF.loc[df['income'] == 0]
    ...
    # colors = ['#e5ac77','#3f4d53']
    for val in unique_val:
        above_fifty = (high_inc.loc[df[column] == val].shape[0]) / (
                    (high_inc.loc[df[column] == val].shape[0]) + (low_inc.loc[df[column] == val].shape[0]))
        percent_above_fifty.append(above_fifty * 100)

    newDF = pd.DataFrame(
        {'Percent of Individuals Earning More Than 50k Annually': percent_above_fifty, column: unique_val})
    newDF = newDF.sort_values(by=column)
    plt.bar(newDF[column], newDF['Percent of Individuals Earning More Than 50k Annually'], width=0.9, color='#e5ac77')
    plt.xlabel(column)
    plt.ylabel("Percent of Individuals Earning More Than 50k Annually")
...
def create_pie(df, column):
    ...
    for val in unique_val:
        ...
        plt.pie([low_inc_count, high_inc_count], labels=['Less Than 50k', 'More Than 50K'], colors=colors, shadow=False,
                autopct='%.2f%%')
...
def load_data():
    ...
    # local adult.data
    try:
        data = np.genfromtxt("https://archive.ics.uci.edu/ml/machine-learning-databases/adult/adult.data",
                             delimiter=', ', dtype=str)
    except:
    ...
    df = df.replace({'marital_status': {'Married-civ-spouse': 'Married', 'Married-AF-spouse': 'Married',
    ...
    df = df.replace({'native_country': {'Outlying-US(Guam-USVI-etc)': 'United-States', '?': 'Other/Unknown',
                                        'South': 'Other/Unknown', 'Puerto-Rico': 'United-States'}})
